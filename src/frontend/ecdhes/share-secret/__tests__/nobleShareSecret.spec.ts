...
import { nobleShareSecret } from '../nobleShareSecret';
...
describe('nobleShareSecret', () => {
  it.each([
    ...
    async (name, createCurve, bitLength) => {
      ...
      const shareSecret = nobleShareSecret(curve);
      ...
      const sharedSecretA = await shareSecret({
        privateKey: privateKeyA,
        publicKey: publicKeyB,
      });
      const sharedSecretB = await shareSecret({
